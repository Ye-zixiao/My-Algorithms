...
/*
		...
		如何在已知next[j]的前提下去推断next[j+1]?
			...
			2、用p[next[k]]去和p[j]进行比较。因为p[k]前面和p[j]前面的字符串部分我们之前就比较过了，知道它们俩相同，而p[k]前又有相同的前后缀（假设）
		，那么p[j]前面也必然有块区域有相同的前后缀。所以可以认为p[next[k]]与p[j]前面的一小块区域已经比较过了且判定为相同，只要继续
		比较p[next[k]]和p[j]就可以确定两个小块区域是否相同，以判定是否有稍小的最大前后缀公共元素长度，否则还要找下一个next[next[k]]，
		直到找到或者没有找到最终变成了0。
